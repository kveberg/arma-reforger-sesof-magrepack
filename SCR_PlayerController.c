...
modded class SCR_PlayerController : PlayerControllerClass
{
	...
	void SESOF_MagRepack_Repack(MagazineComponent fromMag, MagazineComponent toMag, SCR_InventoryStorageManagerComponent managerComp, BaseInventoryStorageComponent storageTo, BaseInventoryStorageComponent storageFrom)
	{
		if (!fromMag || !toMag)
		{
			Print("Could not locate magazine components", LogLevel.DEBUG);
			return;
		}
		
		if (!managerComp)
		{
			Print("Was unable to locate storage or manager components", LogLevel.DEBUG);
			return;
		}
	...
	void SESOF_MagRepack_RpcAsk_Repack(RplId fromMag, RplId toMag, RplId managerComp, RplId storageTo, RplId storageFrom)
	{
  		// Make sure we have everything
		if (!fromMag.IsValid())
		{
			Print("Invalid from magazine", LogLevel.DEBUG);
			return;
		}
		
		if (!toMag.IsValid())
		{
			Print("Invalid to magazine", LogLevel.DEBUG);
			return;
		}
		
		if (!managerComp.IsValid())
		{
			Print("Invalid manager", LogLevel.DEBUG);
			return;
		}
		
		if (!storageTo.IsValid())
		{
			Print("Invalid storageTo", LogLevel.DEBUG);
			return;
		}
		
		if (!storageFrom.IsValid())
		{
			Print("Invalid storageFrom", LogLevel.DEBUG);
			return;
		}
		
		SESOF_MagRepack_Repack(
